...
export const TOOLTIP = {
  ...
  triggerOn: 'mousemove|click',
  className: 'ff-component-stack-bar-chart-626b4131e6dcf01bc1ff6fad-tooltip-wrapper',
  formatter: function (params) {
    ...
    // }
    const { name = '' } = params[0];
    const content = params.map(({ seriesName, color, value }) => {
      return `
        <div class="ff-component-stack-bar-chart-626b4131e6dcf01bc1ff6fad-tooltip-item">
          <p><i style="background: ${color ? typeof color === 'string' ? color : getGradientPreview(color.colorStops).background : 'transparent'}"></i> <span>${seriesName}:</span></p>
          <span>${value}</span>
        </div>`
    })
    const tooltip = `
      <div class="ff-component-stack-bar-chart-626b4131e6dcf01bc1ff6fad-tooltip"><p class="ff-component-stack-bar-chart-626b4131e6dcf01bc1ff6fad-tooltip-title">
        ${name}</p>${content.join('')}
      </div>`
    return tooltip
  },
