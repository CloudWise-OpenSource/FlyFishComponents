   ...
   */
		...
		value: function _render() {
			...
			var _getOptions = this.getOptions(),
			    ...
			    animationSwitch = _getOptions.animationSwitch,
			    animationName = _getOptions.animationName,
			    animationDuration = _getOptions.animationDuration,
			    animationIterationCount = _getOptions.animationIterationCount;
			...
			var pureClassname = this.getContainer().attr('class').replace(/scale|breath|rotate/gi, '');
			var classname = this.getContainer().attr('class').replace(/scale|breath|rotate/gi, '');
			...
			// 加一个延时来保证切换次数是ok的
			setTimeout(function () {
				console.log(_this2.getContainer().attr('class', classname));
				_this2.getContainer().attr('class', classname).css(style);
			}, 200);
...
ImageComponent.enableLoadCssFile = true;
...
Object(data_vi_components__WEBPACK_IMPORTED_MODULE_0__["registerComponent"])('61aa27abd39bdf74f6d60142', 'v-current', _src_Component__WEBPACK_IMPORTED_MODULE_1__["default"]);
